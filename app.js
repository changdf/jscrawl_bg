...
io.sockets.on('connection', function (socket) {
	...
	socket.on('post_message', function (data) {
		...
		for(var i=0;i<data.post_messageList.length;i++){
			...
			/*var querySql = "insert into twitter_user(nickname,url,tag) values('"+message.nickName+"','"+message.url+"','"+message.tag+"')";*/
			var querySql = "insert into twitter_user set ?";
			console.log("nickName"+message.nickName);
			conn.query(querySql,{nickname:message.nickName,url:message.url,tag:message.tag},function(err){
				console.log(err);				
			});
		}
	});
